...
# Load new data
df = pd.read_csv("nexus_ai_dataset_final_3000_multidebug_enforced.csv")
...
# Save encoders
with open("nexus_ai_model_bert/severity_encoder.pkl", "wb") as f:
    pickle.dump(severity_encoder, f)
with open("nexus_ai_model_bert/show_cmd_encoder.pkl", "wb") as f:
    pickle.dump(show_cmd_encoder, f)
with open("nexus_ai_model_bert/debug_cmd_encoder.pkl", "wb") as f:
...
args_show = TrainingArguments(
    output_dir="./nexus_ai_model_bert/show_cmd_model",
    num_train_epochs=4,
...
args_debug = TrainingArguments(
    output_dir="./nexus_ai_model_bert/debug_cmd_model",
    num_train_epochs=4,
    per_device_train_batch_size=8,
    per_device_eval_batch_size=8,
...
tokenizer.save_pretrained("nexus_ai_model_bert")
model_show.save_pretrained("nexus_ai_model_bert/show_cmd_model")
model_debug.save_pretrained("nexus_ai_model_bert/debug_cmd_model")


# Save trained models
import torch
torch.save(model_show, "nexus_ai_model_bert/show_model.pt")
torch.save(model_debug, "nexus_ai_model_bert/debug_model.pt")
